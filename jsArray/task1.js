...
for (let i = 0; i < numbers.length; i++) { // Ітерація через масив для перевірки кожного числа
  const currentNum = numbers[i];           // Поточне число з масиву для перевірки
    if (currentNum > 0) {                  // Перевірка чи число позитивне
        positiveCount++;                   // Збільшення лічильника позитивних чисел на 1
    } else if (currentNum < 0) {           // Перевірка чи число негативне
